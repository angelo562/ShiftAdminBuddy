...
def get_list_dates(fxn_list) -> list[dt.date, dt.date]:
    ...
    datelist_within_range = [
        date for date in datelist if start_date <= date <= end_date
    ]
...
def if_weekends():
    ...
    return sorted(weekends.tolist())


def get_datelist() -> None:
    if data.Data.weekends:
        data.Data.dates_to_query = if_weekends()

    elif data.Data.minor_holidays is True:
        data.Data.dates_to_query = if_all_holidays()

    else:
        data.Data.dates_to_query = if_only_major_holidays()

    # This is the fxn to run if weekends are true
